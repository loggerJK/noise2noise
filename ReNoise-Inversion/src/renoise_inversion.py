    ...
    for _outer in range(num_reg_steps):
        if lambda_kl > 0:
            ...
            if torch.isnan(_grad).any():
                warnings.warn("NaN detected in _grad")
            if torch.isnan(e_t).any():
                warnings.warn("NaN detected in e_t")
            if torch.isnan(_grad).any():
...
def inversion_step(
    ...
    for i in range(num_renoise_steps + 1):
        ...
        if i >= avg_range[0] or (not pipe.cfg.average_latent_estimations and i > 0):
            ...
            if batch_size > 1:
                noise_pred_list = []
                for noise_pred_, noise_pred_optimal_ in zip(noise_pred, noise_pred_optimal):
                    noise_pred_ = noise_regularization(noise_pred_.unsqueeze(0), noise_pred_optimal_.unsqueeze(0), lambda_kl=pipe.cfg.noise_regularization_lambda_kl, lambda_ac=pipe.cfg.noise_regularization_lambda_ac, num_reg_steps=pipe.cfg.noise_regularization_num_reg_steps, num_ac_rolls=pipe.cfg.noise_regularization_num_ac_rolls, generator=generator)
                    noise_pred_list.append(noise_pred_.squeeze())
    ...
    if pipe.cfg.average_latent_estimations and nosie_pred_avg is not None:
        batch_size = nosie_pred_avg.shape[0]
        if batch_size > 1:
            noise_pred_list = []
            for noise_pred_, noise_pred_optimal_ in zip(nosie_pred_avg, noise_pred_optimal):
                noise_pred_ = noise_regularization(noise_pred_.unsqueeze(0), noise_pred_optimal_.unsqueeze(0), lambda_kl=pipe.cfg.noise_regularization_lambda_kl, lambda_ac=pipe.cfg.noise_regularization_lambda_ac, num_reg_steps=pipe.cfg.noise_regularization_num_reg_steps, num_ac_rolls=pipe.cfg.noise_regularization_num_ac_rolls, generator=generator)
                noise_pred_list.append(noise_pred_.squeeze())
            nosie_pred_avg = torch.stack(noise_pred_list)
        else:
