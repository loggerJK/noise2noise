...
def main(args):
    ...
    save_dir_initial = f'./{args.name}/initial/'
    save_dir_inversion = f'./{args.name}/inversion/'
    ...
    cap = torch.utils.data.Subset(cap, range(start_idx, start_idx + num_prompt_per_gpu))
    print(f"GPU {rank} processing {start_idx} to {start_idx + num_prompt_per_gpu - 1}")
    ...
    for i, (input_image, captions) in tqdm(enumerate(cap)): # Prompt, Image
        for j, seed in enumerate(random_seeds): # Random Seed
            torch.manual_seed(seed)
            ...
            i = i + start_idx # i : 0 ~ 999, Real prompt index
