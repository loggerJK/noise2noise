...
class Trainer(Module):
    ...
    def load(self, path):
        ...
        self.model.module.load_state_dict(load_package["model"])
    ...
    def forward(self):
        ...
        progress_bar = tqdm(total=self.num_train_steps, disable=not self.accelerator.is_local_main_process)
            ...
            if self.is_main:
                ...
                if divisible_by(step, self.save_results_every):
                    ...
                    if self.accelerator.is_local_main_process:
                        sampled = self.model.sample(batch_size=1, data_shape=data[0].shape[1:])
                        img = self.val_using_sdxl(sampled)
                        self.accelerator.log({"image": wandb.Image(img)}, step = step)
    ...
    def val_using_sdxl(self, latent):
        ...
        with torch.no_grad():
            img = pipe(
                prompt = "Closeup of bins of food that include broccoli and bread.",
                num_inference_steps=30,
                guidance_scale = 0.0,
                latents = latent.to(torch.float16).to(self.accelerator.device),
            ).images[0]
